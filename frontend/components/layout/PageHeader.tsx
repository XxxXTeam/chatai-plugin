...
export function PageHeader({ title, description, icon: Icon, actions, className }: PageHeaderProps) {
    return (
        <div
            className={cn(
                'flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between pb-6 animate-fade-in',
                className
            )}
        >
            <div className="flex items-center gap-4">
                ...
                <div className="space-y-1">
                    ...
                    {description && <p className="text-muted-foreground/80 text-sm font-medium">{description}</p>}
                </div>
            </div>
