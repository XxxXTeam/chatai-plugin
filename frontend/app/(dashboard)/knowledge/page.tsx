...
export default function KnowledgePage() {
    ...
    const handleOpenDialog = async (doc?: KnowledgeDocument) => {
        if (doc) {
            ...
            // 如果内容被截断，获取完整内容
            if (
                (doc as { truncated?: boolean }).truncated ||
                ((doc as { contentLength?: number }).contentLength &&
                    (doc as { contentLength: number }).contentLength > 500)
            ) {
                try {
    ...
    const handleFileSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            ...
            if (file.name.endsWith('.json')) {
                ...
                const hasDocs =
                    isObject &&
                    'docs' in (content as Record<string, unknown>) &&
                    Array.isArray((content as Record<string, unknown>).docs)
    ...
    const handleImport = async () => {
        ...
        try {
            ...
            if (stats) {
                toast.success(
                    `导入成功！共 ${stats.imported} 条记录，${stats.entityCount} 个实体，${stats.tripleCount} 个关系`
                )
            } else {
                toast.success('导入成功')
            }
    ...
    const filteredDocuments = searchQuery ? documents : documents
                ...
                <DialogContent>
                    ...
                    <div className="space-y-2 max-h-60 overflow-y-auto">
                        ...
                        {presets.filter(p => !p.isBuiltin).length === 0 && (
                            <p className="text-sm text-muted-foreground text-center py-4">暂无可关联的自定义预设</p>
                        )}
                    </div>
                </DialogContent>
            </Dialog>
                    ...
                    {filteredDocuments.map(doc => (
                        <Card key={doc.id}>
                            <CardHeader className="pb-3">
                                ...
                                <CardDescription>更新于 {formatDate(doc.updatedAt)}</CardDescription>
                            </CardHeader>
                            <CardContent className="space-y-3">
                                ...
                                {(doc as { contentLength?: number }).contentLength &&
                                    (doc as { contentLength: number }).contentLength > 0 && (
                                        <div className="text-xs text-muted-foreground">
                                            内容长度:{' '}
                                            {((doc as { contentLength: number }).contentLength / 1000).toFixed(1)}K 字符
                                        </div>
                                    )}
            ...
            <Dialog open={importDialogOpen} onOpenChange={setImportDialogOpen}>
                <DialogContent className="w-[95vw] max-w-lg">
                    ...
                    <div className="space-y-4">
                        <div className="grid gap-2">
                            <Label>选择文件</Label>
                            <Input type="file" accept=".json,.txt,.md" onChange={handleFileSelect} />
                            {importForm.fileName && (
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            ...
                            <div className="grid gap-2">
                                ...
                                <Select
                                    ...
                                    onValueChange={(v: 'create' | 'merge' | 'replace') =>
                                        setImportForm({ ...importForm, mergeMode: v })
                                    }
                                >
                                    <SelectTrigger>
                                        <SelectValue />
