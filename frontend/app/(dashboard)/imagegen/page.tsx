...
export default function ImageGenPage() {
    ...
    const loadData = async (showLoading = true) => {
        ...
        try {
            const [presetsRes, configRes] = await Promise.all([imageGenApi.getPresets(), imageGenApi.getConfig()])
    ...
    const handleAddPreset = async () => {
        ...
        try {
            ...
            const data: {
                ...
                prompt: string
                needImage: boolean
                splitGrid?: { cols: number; rows: number; padding: number }
            } = {
                keywords,
            ...
            if (newPreset.enableSplit) {
                data.splitGrid = {
                    cols: newPreset.splitCols,
                    rows: newPreset.splitRows,
                    padding: newPreset.splitPadding
                }
            }
    ...
    const handleSavePreset = async () => {
        ...
        try {
            ...
            const data: {
                ...
                prompt: string
                needImage: boolean
                splitGrid?: { cols: number; rows: number; padding: number } | null
            } = {
                keywords,
            ...
            if (editingPreset.enableSplit) {
                data.splitGrid = {
                    cols: editingPreset.splitCols,
                    rows: editingPreset.splitRows,
                    padding: editingPreset.splitPadding || 0
                }
            } else {
                data.splitGrid = null
            }
        ...
        <div className="container mx-auto px-4 sm:px-6 py-4 sm:py-6 space-y-4 sm:space-y-6">
            ...
            <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-4">
                <TabsList className="w-full sm:w-auto flex flex-wrap h-auto gap-1 p-1">
                    <TabsTrigger value="presets" className="flex-1 sm:flex-none text-xs sm:text-sm">
                        预设列表
                    </TabsTrigger>
                    <TabsTrigger value="sources" className="flex-1 sm:flex-none text-xs sm:text-sm">
                        预设来源
                    </TabsTrigger>
                    <TabsTrigger value="custom" className="flex-1 sm:flex-none text-xs sm:text-sm">
                        自定义
                    </TabsTrigger>
                                                    ...
                                                    <TableCell>
                                                        {preset.splitGrid ? (
                                                            <Badge variant="secondary">
                                                                ...
                                                                {preset.splitGrid.cols}×{preset.splitGrid.rows}
                                                            </Badge>
                                                        ) : (
                                                        ...
                                                        <div className="flex gap-1">
                                                            ...
                                                            {preset.source === 'custom' && (
                                                                <Button
                                                                    ...
                                                                    onClick={() => {
                                                                        const customIndex =
                                                                            config?.customPresets?.findIndex(
                                                                                p => p.uid === preset.uid
                                                                            )
                                                                        if (
                                                            ...
                                                            {preset.source === 'builtin' && preset.uid && (
                                                                <Button
                                                                    ...
                                                                    size="icon"
                                                                    onClick={() =>
                                                                        handleDeleteBuiltinPreset(preset.uid!)
                                                                    }
                                                                >
                                                                    <Trash2 className="h-4 w-4 text-destructive" />
                                                                </Button>
                                ...
                                <TableBody>
                                    {config?.customPresets?.map((preset, index) => (
                                        <TableRow key={index}>
                                            ...
                                            <TableCell>
                                                {preset.needImage ? (
                                                    <Badge variant="secondary">需要</Badge>
                                                ) : (
                                                    <Badge variant="outline">不需要</Badge>
                                                )}
                                            </TableCell>
                                            ...
                                            <Button
                                                ...
                                                onClick={async () => {
                                                    ...
                                                    try {
                                                        ...
                                                        const models =
                                                            (res as { data?: { models?: string[] } })?.data?.models ||
                                                            []
                                                        const apis = [...(config?.apis || [])]
                                    ...
                                    <Collapsible open={imageModelSelectorOpen} onOpenChange={setImageModelSelectorOpen}>
                                        <CollapsibleTrigger asChild>
                                            <Button variant="outline" className="w-full justify-between">
                                                ...
                                                <Settings
                                                    className={`h-4 w-4 shrink-0 transition-transform ${imageModelSelectorOpen ? 'rotate-90' : ''}`}
                                                />
                                            </Button>
                                        </CollapsibleTrigger>
                                    ...
                                    <Collapsible open={videoModelSelectorOpen} onOpenChange={setVideoModelSelectorOpen}>
                                        <CollapsibleTrigger asChild>
                                            <Button variant="outline" className="w-full justify-between">
                                                ...
                                                <Settings
                                                    className={`h-4 w-4 shrink-0 transition-transform ${videoModelSelectorOpen ? 'rotate-90' : ''}`}
                                                />
                                            </Button>
                                        </CollapsibleTrigger>
                                        <CollapsibleContent className="mt-2">
                            ...
                            <div className="grid gap-4 md:grid-cols-2">
                                <div className="space-y-2">
                                    ...
                                    <Input
                                        ...
                                        onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
                                            setConfig(c =>
                                                c ? { ...c, timeout: parseInt(e.target.value) || 600000 } : c
                                            )
                                        }
                                ...
                                <div className="space-y-2">
                                    ...
                                    <Input
                                        ...
                                        onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
                                            setConfig(c => (c ? { ...c, maxImages: parseInt(e.target.value) || 3 } : c))
                                        }
            ...
            <Dialog open={addPresetOpen} onOpenChange={setAddPresetOpen}>
                <DialogContent className="w-[95vw] max-w-2xl">
                        ...
                        <div className="space-y-3">
                            <div className="flex items-center justify-between">
                                ...
                                {newPreset.enableSplit && (
                                    <Badge variant="secondary">
                                        <Grid3X3 className="w-3 h-3 mr-1" />
                                        {newPreset.splitCols}×{newPreset.splitRows} ={' '}
                                        {newPreset.splitCols * newPreset.splitRows}个
                                    </Badge>
                                )}
                            </div>
                            {newPreset.enableSplit && (
                                <div className="p-4 border rounded-lg bg-muted/30 space-y-4">
                                    <div className="flex items-start gap-2 p-2 bg-blue-50 dark:bg-blue-950/30 rounded text-xs text-blue-700 dark:text-blue-300">
                                        ...
                                        <div>
                                            ...
                                            <p>
                                                生成的图片将按网格切割成多个小图，通常用于Q版表情包生成。建议在提示词中指定生成
                                                4K/16:9 图片以获得最佳效果。
                                            </p>
                                        </div>
                                    </div>
                                        ...
                                        <div className="space-y-2">
                                            ...
                                            <Input
                                                ...
                                                onChange={e =>
                                                    setNewPreset(p => ({
                                                        ...p,
                                                        splitCols: parseInt(e.target.value) || 6
                                                    }))
                                                }
                                            />
                                        </div>
                                        <div className="space-y-2">
                                            ...
                                            <Input
                                                ...
                                                onChange={e =>
                                                    setNewPreset(p => ({
                                                        ...p,
                                                        splitRows: parseInt(e.target.value) || 4
                                                    }))
                                                }
                                            />
                                        </div>
                                        <div className="space-y-2">
                                            ...
                                            <Input
                                                ...
                                                onChange={e =>
                                                    setNewPreset(p => ({
                                                        ...p,
                                                        splitPadding: parseInt(e.target.value) || 0
                                                    }))
                                                }
                                            />
                                        </div>
                                    ...
                                    <div className="grid grid-cols-2 gap-4 text-sm">
                                        ...
                                        <div className="p-2 bg-muted rounded">
                                            ...
                                            <span className="font-medium ml-1">
                                                约 {Math.floor(3840 / newPreset.splitCols)}×
                                                {Math.floor(2160 / newPreset.splitRows)} px
                                            </span>
                                        </div>
                                    </div>
                                </div>
                    ...
                    <DialogFooter>
                        <Button variant="outline" onClick={() => setAddPresetOpen(false)}>
                            取消
                        </Button>
                        ...
                        <div className="space-y-4">
                            <div className="space-y-2">
                                ...
                                <Input
                                    ...
                                    onChange={e =>
                                        setEditingPreset(p => (p ? { ...p, keywords: e.target.value } : null))
                                    }
                                    placeholder="用逗号或空格分隔，例如：风景化, 风景"
                                />
                                <p className="text-xs text-muted-foreground">用户发送这些关键词会触发此预设</p>
                            ...
                            <div className="flex items-center space-x-2">
                                <Switch
                                    ...
                                    onCheckedChange={checked =>
                                        setEditingPreset(p => (p ? { ...p, needImage: checked } : null))
                                    }
                                />
                                <Label>需要用户提供图片</Label>
                            </div>
                                ...
                                <div className="flex items-center justify-between">
                                    <div className="flex items-center space-x-2">
                                        <Switch
                                            ...
                                            onCheckedChange={checked =>
                                                setEditingPreset(p => (p ? { ...p, enableSplit: checked } : null))
                                            }
                                        />
                                        <Label>启用图片切割（表情包模式）</Label>
                                    </div>
                                    {editingPreset.enableSplit && (
                                        <Badge variant="secondary">
                                            <Grid3X3 className="w-3 h-3 mr-1" />
                                            {editingPreset.splitCols}×{editingPreset.splitRows} ={' '}
                                            {editingPreset.splitCols * editingPreset.splitRows}个
                                        </Badge>
                                    )}
                                </div>
                                {editingPreset.enableSplit && (
                                    <div className="p-4 border rounded-lg bg-muted/30 space-y-4">
                                            ...
                                            <div className="space-y-2">
                                                ...
                                                <Input
                                                    ...
                                                    onChange={e =>
                                                        setEditingPreset(p =>
                                                            p
                                                                ? { ...p, splitCols: parseInt(e.target.value) || 6 }
                                                                : null
                                                        )
                                                    }
                                                />
                                            ...
                                            <div className="space-y-2">
                                                ...
                                                <Input
                                                    ...
                                                    onChange={e =>
                                                        setEditingPreset(p =>
                                                            p
                                                                ? { ...p, splitRows: parseInt(e.target.value) || 4 }
                                                                : null
                                                        )
                                                    }
                                                />
                                            ...
                                            <div className="space-y-2">
                                                ...
                                                <Input
                                                    ...
                                                    onChange={e =>
                                                        setEditingPreset(p =>
                                                            p
                                                                ? { ...p, splitPadding: parseInt(e.target.value) || 0 }
                                                                : null
                                                        )
                                                    }
                                                />
                                        ...
                                        <div className="grid grid-cols-2 gap-4 text-sm">
                                            ...
                                            <div className="p-2 bg-muted rounded">
                                                <span className="text-muted-foreground">单个尺寸(4K)：</span>
                                                <span className="font-medium ml-1">
                                                    约 {Math.floor(3840 / editingPreset.splitCols)}×
                                                    {Math.floor(2160 / editingPreset.splitRows)} px
                                                </span>
                                            </div>
                                        </div>
                                    </div>
