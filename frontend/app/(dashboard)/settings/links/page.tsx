...
export default function LoginLinksPage() {
    ...
    const fetchConfig = async () => {
        try {
            setLoading(true)
            const res = (await configApi.get()) as {
                data?: { web?: { loginLinks?: LoginLink[]; permanentAuthToken?: string; publicUrl?: string } }
            }
        ...
        <div className="space-y-6">
            ...
            <Card>
                <CardHeader>
                    ...
                    <CardDescription>永久Token可用于生成不过期的登录链接，请妥善保管</CardDescription>
                </CardHeader>
                <CardContent className="space-y-4">
                    {permanentToken ? (
                        <>
                            ...
                            <div className="flex items-center gap-2">
                                <Input value={permanentToken} readOnly className="font-mono text-sm" />
                                <Button
                                    variant="outline"
                                    size="icon"
                            ...
                            <div className="flex gap-2">
                                ...
                                <Button variant="destructive" onClick={handleRevokeToken} disabled={tokenLoading}>
                                    {tokenLoading ? (
                                        <Loader2 className="h-4 w-4 mr-2 animate-spin" />
                                    ) : (
            ...
            <Card>
                <CardHeader>
                    ...
                    <CardDescription>手动配置公网访问地址，用于 #ai管理面板 命令显示</CardDescription>
                </CardHeader>
                <CardContent className="space-y-4">
                    <div className="flex items-center gap-2">
                        <Input
                        ...
                        <Button onClick={handleSavePublicUrl} disabled={savingPublicUrl}>
                            {savingPublicUrl ? <Loader2 className="h-4 w-4 animate-spin" /> : '保存'}
                        </Button>
                    </div>
                    <Alert>
            ...
            <Card>
                <CardHeader>
                    ...
                    <CardDescription>添加自定义域名或地址，用于生成对应的登录链接</CardDescription>
                </CardHeader>
                <CardContent className="space-y-4">
                    {!permanentToken && (
                        <Alert>
                            <Info className="h-4 w-4" />
                            <AlertDescription>请先生成永久Token，才能使用自定义登录链接</AlertDescription>
                        </Alert>
                    )}
