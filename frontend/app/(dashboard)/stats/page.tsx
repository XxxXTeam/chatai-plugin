...
export default function StatsPage() {
    ...
    const fetchStats = async () => {
        try {
            setLoading(true)
            const [statsRes, usageRes] = await Promise.all([statsApi.getOverview(), usageStatsApi.getRecent(50)])
        ...
        <div className="space-y-6">
                        ...
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                ...
                                <p className="text-xs text-muted-foreground">
                                    {stats?.messages.conversations || 0} 个对话
                                </p>
                            </div>
                        </div>
                    </CardContent>
                    ...
                    <CardContent className="p-6">
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                ...
                                <p className="text-xs text-muted-foreground">
                                    {stats?.models.byModel?.length || 0} 个模型
                                </p>
                            </div>
                        </div>
                    </CardContent>
                    ...
                    <CardContent className="p-6">
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                ...
                                <p className="text-xs text-muted-foreground">
                                    入 {formatNumber(stats?.tokens.total?.input || 0)} / 出{' '}
                                    {formatNumber(stats?.tokens.total?.output || 0)}
                                </p>
                            </div>
                        </div>
                    </CardContent>
                    ...
                    <CardContent className="p-6">
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                ...
                                <p className="text-xs text-muted-foreground">
                                    {stats?.tools.byTool?.length || 0} 个工具
                                </p>
                            </div>
                        </div>
                    </CardContent>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                                    ...
                                    {stats.tools.byTool.map((tool, idx) => (
                                        ...
                                        >
                                            <div className="flex-1 min-w-0">
                                                ...
                                                <p className="text-xs text-muted-foreground">
                                                    成功率{' '}
                                                    {tool.calls > 0 ? Math.round((tool.success / tool.calls) * 100) : 0}
                                                    %
                                                </p>
                                            </div>
                                            <div className="text-right">
                                                ...
                                                <Badge
                                                    variant={tool.failed > 0 ? 'destructive' : 'secondary'}
                                                    className="text-xs"
                                                >
                ...
                <Card className="lg:col-span-2">
                    ...
                    <CardContent>
                        {stats?.messages.hourlyDistribution &&
                        Object.keys(stats.messages.hourlyDistribution).length > 0 ? (
                                                        ...
                                                        <div>
                                                            ...
                                                            <div className="bg-muted/50 p-2 rounded text-xs space-y-1">
                                                                {record.request.messageCount && (
                                                                    <p>消息数: {record.request.messageCount}</p>
                                                                )}
                                                                {record.request.systemPromptLength && (
                                                                    <p>
                                                                        系统提示长度:{' '}
                                                                        {record.request.systemPromptLength}
                                                                    </p>
                                                                ...
                                                                {record.request.lastUserMessage && (
                                                                    <p className="truncate">
                                                                        最后用户消息: {record.request.lastUserMessage}
                                                                    </p>
                                                                )}
                                                                {record.request.hasImages && (
                                                                    <Badge variant="outline" className="text-xs">
                                                                        包含图片
                                                                    </Badge>
                                                                )}
                                                                {record.request.hasTools && (
                                                                    <Badge variant="outline" className="text-xs">
                                                                        使用工具 ({record.request.toolCount})
                                                                    </Badge>
                                                                )}
                                                            </div>
                                                        </div>
