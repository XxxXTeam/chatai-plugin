    ...
    const handleOpenDialog = (channel?: Channel) => {
        if (channel) {
            ...
            setForm({
                ...
                requestBodyTemplate:
                    ((channel as unknown as Record<string, unknown>).requestBodyTemplate as string) || '',
                advanced: deepMerge(
                    { ...defaultAdvanced },
            ...
            if (Array.isArray(models) && models.length > 0) {
                ...
                const modelIds = models
                    .map((m: unknown) =>
                        typeof m === 'string'
                            ? m
                            : (m as Record<string, string>)?.id || (m as Record<string, string>)?.name
                    )
                    .filter(Boolean)
                setAvailableModels(modelIds)
                // 设置当前已选模型
                const currentModels = form.models
                    .split(',')
                    .map(m => m.trim())
                    .filter(Boolean)
                                            ...
                                            <div className="grid gap-2">
                                                ...
                                                <Select
                                                    onValueChange={key => {
                                                        ...
                                                        // 根据预设类型显示不同提示
                                                        const presetHints: Record<
                                                            string,
                                                            { type: 'success' | 'info'; message: string; url?: string }
                                                        > = {
                                                            'free-api': {
                                                                type: 'success',
                                                                message:
                                                                    '作者内置免费API,使用前请手动点击获取模型更新模型列表'
                                                            },
                                                            'free-glm': { type: 'success', message: '免费GLM' },
                                                            'free-xiaomi': { type: 'success', message: '免费小米MiMo' },
                                                            'free-gemini2': {
                                                                type: 'success',
                                                                message: '免费Gemini对话API'
                                                            },
                                                            'free-gemini': {
                                                                type: 'info',
                                                            ...
                                                            'free-grok': {
                                                                type: 'info',
                                                                message: '免费Grok - 需在 x.ai 获取API Key'
                                                            },
                                                            'free-opencode': {
                                                                type: 'success',
                                                                message: '免费opencode'
                                                            ...
                                                            openai: {
                                                                type: 'info',
                                                                message:
                                                                    'OpenAI - 请填入API Key（sk-xxx），可在 platform.openai.com 获取'
                                                            },
                                                            gemini: {
                                                                type: 'info',
                                                                message:
                                                                    'Gemini - 请填入API Key，可在 aistudio.google.com 获取'
                                                            },
                                                            claude: {
                                                                type: 'info',
                                                                message:
                                                                    'Claude - 请填入API Key，可在 console.anthropic.com 获取'
                                                            },
                                                            grok: {
                                                                type: 'info',
                                                                message: 'Grok - 请填入API Key，可在 console.x.ai 获取'
                                                            },
                                                            mistral: {
                                                                type: 'info',
                                                                message:
                                                                    'Mistral AI - 请填入API Key，可在 console.mistral.ai 获取'
                                                            },
                                                            groq: {
                                                                type: 'info',
                                                                message:
                                                                    'Groq - 请填入API Key，可在 console.groq.com 免费获取'
                                                            },
                                                            deepseek: {
                                                                type: 'info',
                                                                message:
                                                                    'DeepSeek - 请填入API Key，可在 platform.deepseek.com 获取'
                                                            },
                                                            zhipu: {
                                                                type: 'info',
                                                                message:
                                                                    '智谱AI - 请填入API Key，可在 open.bigmodel.cn 获取'
                                                            },
                                                            qwen: {
                                                                type: 'info',
                                                                message:
                                                                    '通义千问 - 请填入API Key，可在 dashscope.console.aliyun.com 获取'
                                                            },
                                                            moonshot: {
                                                                type: 'info',
                                                                message:
                                                                    'Kimi - 请填入API Key，可在 platform.moonshot.cn 获取'
                                                            },
                                                            minimax: {
                                                                type: 'info',
                                                                message:
                                                                    'MiniMax - 请填入API Key，可在 platform.minimaxi.com 获取'
                                                            },
                                                            yi: {
                                                                type: 'info',
                                                                message:
                                                                    '零一万物 - 请填入API Key，可在 platform.lingyiwanwu.com 获取'
                                                            },
                                                            baichuan: {
                                                                type: 'info',
                                                                message:
                                                                    '百川智能 - 请填入API Key，可在 platform.baichuan-ai.com 获取'
                                                            },
                                                            doubao: {
                                                                type: 'info',
                                                                message:
                                                                    '豆包 - 需配置endpoint_id，可在 console.volcengine.com 获取'
                                                            },
                                                            spark: {
                                                                type: 'info',
                                                                message:
                                                                    '讯飞星火 - 请填入APIKey:APISecret格式，可在 xinghuo.xfyun.cn 获取'
                                                            },
                                                            openrouter: {
                                                                type: 'info',
                                                                message:
                                                                    'OpenRouter - 请填入API Key，可在 openrouter.ai 获取'
                                                            },
                                                            siliconflow: {
                                                                type: 'info',
                                                                message:
                                                                    '硅基流动 - 请填入API Key，可在 cloud.siliconflow.cn 获取'
                                                            },
                                                            together: {
                                                                type: 'info',
                                                                message:
                                                                    'Together AI - 请填入API Key，可在 api.together.xyz 获取'
                                                            }
                                                        }
                                                    ...
                                                    <TooltipProvider>
                                                        <Tooltip>
                                                            ...
                                                            <TooltipContent side="right" className="max-w-[300px]">
                                                                ...
                                                                <p className="text-xs">
                                                                    启用后可配置多个 API
                                                                    Key，支持轮询、随机、权重等策略自动切换，用于负载均衡和故障转移。
                                                                </p>
                                                            </TooltipContent>
                                                        </Tooltip>
                                                    </TooltipProvider>
                                            ...
                                            {form.useMultiKey && (
                                                <div className="space-y-3 p-3 border rounded-lg bg-muted/30">
                                                    <div className="flex items-start gap-2 p-2 bg-blue-50 dark:bg-blue-950/30 rounded text-xs text-blue-700 dark:text-blue-300">
                                                        ...
                                                        <div>
                                                            ...
                                                            <ul className="mt-1 space-y-0.5 text-blue-600 dark:text-blue-400">
                                                                <li>
                                                                    • <strong>轮询</strong>：按顺序依次使用每个Key
                                                                </li>
                                                                <li>
                                                                    • <strong>随机</strong>：每次随机选择一个Key
                                                                </li>
                                                                <li>
                                                                    • <strong>权重</strong>
                                                                    ：根据权重值随机分配（权重越高使用概率越大）
                                                                </li>
                                                                <li>
                                                                    • <strong>最少使用</strong>
                                                                    ：优先使用调用次数最少的Key
                                                    ...
                                                    <ApiKeyManager
                                                        ...
                                                        onChange={(keys, strategy) =>
                                                            setForm({ ...form, apiKeys: keys, strategy })
                                                        }
                                                ...
                                                <Input
                                                    ...
                                                    onKeyDown={e => {
                                                        if (e.key === 'Enter' && customModelInput.trim()) {
                                                            e.preventDefault()
                                                            const newModels = customModelInput
                                                                .split(',')
                                                                .map(m => m.trim())
                                                                .filter(Boolean)
                                                            const currentModels = form.models
                                                                ...
                                                                .map(m => m.trim())
                                                                .filter(Boolean)
                                                            const uniqueNew = newModels.filter(
                                                                m => !currentModels.includes(m)
                                                            )
                                                            if (uniqueNew.length > 0) {
                                                                setForm({
                                                                    ...form,
                                                                    models: [...currentModels, ...uniqueNew].join(', ')
                                                                })
                                                            }
                                                            setCustomModelInput('')
                                                        }
                                                ...
                                                <div className="space-y-3 p-3 border rounded-lg">
                                                    ...
                                                    <div className="grid gap-2">
                                                        ...
                                                        <Select
                                                            ...
                                                            onValueChange={(value: 'base64' | 'url' | 'auto') =>
                                                                setForm({
                                                                    ...form,
                                                                    imageConfig: {
                                                                        ...form.imageConfig,
                                                    ...
                                                    {form.imageConfig.convertFormat && (
                                                        <div className="grid gap-2">
                                                            ...
                                                            <Select
                                                                ...
                                                                onValueChange={(value: 'png' | 'jpeg' | 'auto') =>
                                                                    setForm({
                                                                        ...form,
                                                                        imageConfig: {
                                                                            ...form.imageConfig,
                                                        ...
                                                        <Slider
                                                            ...
                                                            onValueChange={v =>
                                                                setForm({
                                                                    ...
                                                                    advanced: {
                                                                        ...form.advanced,
                                                                        llm: { ...form.advanced.llm, temperature: v[0] }
                                                                    }
                                                                })
                                                            }
                                                        />
                                                        ...
                                                        <Slider
                                                            ...
                                                            onValueChange={v =>
                                                                setForm({
                                                                    ...
                                                                    advanced: {
                                                                        ...form.advanced,
                                                                        llm: { ...form.advanced.llm, topP: v[0] }
                                                                    }
                                                                })
                                                            }
                                                        />
                                                            ...
                                                            <div className="space-y-2">
                                                                ...
                                                                <textarea
                                                                    ...
                                                                    onChange={e =>
                                                                        setForm({
                                                                            ...form,
                                                                            headersTemplate: e.target.value
                                                                        })
                                                                    }
                                                                />
                                                            </div>
                                                ...
                                                <Tooltip>
                                                    <TooltipTrigger asChild>
                                                        <Badge variant="outline" className="text-xs cursor-help">
                                                            ...
                                                            {channel.apiKeys.filter(k => k.enabled !== false).length}/
                                                    ...
                                                    <TooltipContent>
                                                        ...
                                                        <p className="text-xs">
                                                            策略:{' '}
                                                            {keyStrategies.find(s => s.value === channel.strategy)
                                                                ?.label || channel.strategy}
                                                        </p>
                                                        <p className="text-xs">
                                                            启用:{' '}
                                                            {channel.apiKeys.filter(k => k.enabled !== false).length} /
                                                            总数: {channel.apiKeys.length}
                                                        </p>
                                                    </TooltipContent>
                                                </Tooltip>
                                            </TooltipProvider>
                                            ...
                                            <TooltipProvider>
                                                <Tooltip>
                                                    ...
                                                    <TooltipContent>
                                                        <p>
                                                            成功率:{' '}
                                                            {channel.stats.successCalls && channel.stats.totalCalls
                                                                ? Math.round(
                                                                      (channel.stats.successCalls /
                                                                          channel.stats.totalCalls) *
                                                                          100
                                                                  )
                                                                : 0}
            ...
            <Dialog open={modelSelectorOpen} onOpenChange={setModelSelectorOpen}>
                <DialogContent className="w-[95vw] max-w-2xl max-h-[90vh] sm:max-h-[80vh] flex flex-col">
                    <DialogHeader className="flex-shrink-0">
                        ...
                        <DialogDescription>从列表中选择需要使用的模型</DialogDescription>
                    </DialogHeader>
