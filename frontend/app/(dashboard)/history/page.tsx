...
export default function HistoryPage() {
    ...
    const avgDuration = logs.length > 0 ? Math.round(logs.reduce((acc, l) => acc + l.duration, 0) / logs.length) : 0
            ...
            {filteredLogs.length === 0 ? (
                <Card>
                    <CardContent className="flex flex-col items-center justify-center py-12">
                        ...
                        <p className="text-muted-foreground">{logs.length === 0 ? '暂无调用记录' : '没有匹配的记录'}</p>
                    </CardContent>
                </Card>
            ) : (
            ...
            <Dialog open={!!selectedLog} onOpenChange={() => setSelectedLog(null)}>
                <DialogContent className="w-[95vw] max-w-2xl max-h-[80vh]">
                    <DialogHeader>
                        ...
                        <DialogDescription>
                            {selectedLog && formatTime(selectedLog.timestamp)} · 耗时{' '}
                            {selectedLog && formatDuration(selectedLog.duration)}
                        </DialogDescription>
                    </DialogHeader>
