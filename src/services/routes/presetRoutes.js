...
class ChaiteResponse {
    ...
    static ok(data) { return new ChaiteResponse(0, data, 'ok') }
    static fail(data, msg) { return new ChaiteResponse(-1, data, msg) }
}
...
export function createPresetRoutes(authMiddleware) {
    ...
    router.post('/:id/default', ...auth, asyncHandler(async (req, res) => {
        await ensureInit()
        const preset = presetManager.get(req.params.id)
        if (!preset) {
    ...
    router.get('/:id/prompt', ...auth, asyncHandler(async (req, res) => {
        await ensureInit()
...
export function createPresetsConfigRoutes(authMiddleware) {
    ...
    router.get('/builtin', ...auth, asyncHandler(async (req, res) => {
        await presetManager.init()
