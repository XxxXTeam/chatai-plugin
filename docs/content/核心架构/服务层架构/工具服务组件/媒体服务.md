...
graph TB
subgraph "后端服务"
IS["ImageService<br/>图片处理与存储"]
RS["RenderService<br/>渲染服务"]
RC["RedisClient<br/>缓存客户端"]
CFG["config.js<br/>配置中心"]
IR["imageRoutes.js<br/>图像生成路由"]
MT["media.js<br/>MCP媒体工具"]
end
subgraph "前端集成"
FE["前端管理面板<br/>Next.js应用"]
end
FE --> IR
IR --> IS
IR --> RS
...
```mermaid
flowchart TD
Start(["开始"]) --> CheckInput["检查输入URL"]
CheckInput --> IsBase64{"是否Base64?"}
IsBase64 --> |是| ReturnBase64["返回Base64"]
IsBase64 --> |否| IsLocal{"是否本地文件?"}
IsLocal --> |是| ValidateLocal["验证本地文件存在"]
ValidateLocal --> ReturnLocal["返回本地文件"]
IsLocal --> |否| CheckCache["检查URL缓存"]
CheckCache --> CacheHit{"缓存命中?"}
CacheHit --> |是| ReturnCached["返回缓存结果"]
CacheHit --> |否| FetchURL["HTTP请求验证"]
FetchURL --> ValidateResp["验证响应类型/大小"]
ValidateResp --> IsImage{"是否图片?"}
IsImage --> |是| ReturnValid["返回有效结果"]
IsImage --> |否| ConvertIfNeeded["是否需要转换为Base64?"]
ConvertIfNeeded --> |是| ToBase64["下载并转换为Base64"]
ConvertIfNeeded --> |否| ReturnInvalid["返回无效结果"]
ReturnBase64 --> End(["结束"])
ReturnLocal --> End
ReturnCached --> End
ReturnValid --> End
...
RS->>Browser : 截图生成图片
Browser-->>RS : 返回图片Buffer
RS-->>Client : 返回图片Buffer
```
...
```mermaid
graph TB
IS["ImageService"] --> Sharp["sharp库"]
IS --> Redis["ioredis"]
RS["RenderService"] --> Marked["marked解析器"]
RS --> Puppeteer["puppeteer浏览器"]
