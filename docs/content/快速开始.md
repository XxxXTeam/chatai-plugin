...
本指南帮助你在 Yunzai-Bot 环境中快速完成 ChatAI 插件的安装、构建、启动与初始配置。
...
- 基础配置：通过初始化引导向导完成渠道、模型、预设、触发方式配置

### 初始化引导向导

首次进入 Web 管理面板时，会自动弹出**初始化引导向导**，帮助您快速完成基础配置：
...
> **提示**：使用 OpenEL 免费渠道可快速体验，无需注册账号。配置完成后，面板右上角的 `?` 图标可随时查看功能引导。
