...
function handleModelSelect(models) {
  ...
  // 获取选中的模型，如果没有选择则使用空字符串（清空）
  const selectedModel = models.length > 0 ? models[0] : ''
  
  if (target === 'bymModel') {
    config.bym.model = selectedModel
  } else if (target.startsWith('models.')) {
    // 新的模型分类配置
    const key = target.replace('models.', '')
    config.llm.models[key] = selectedModel
  } else {
    config.llm[target] = selectedModel
  }
  showModelSelector.value = false
}

// 清空指定模型配置
function clearModel(target) {
  if (target === 'bymModel') {
    config.bym.model = ''
  } else if (target.startsWith('models.')) {
    const key = target.replace('models.', '')
    config.llm.models[key] = ''
  } else {
    config.llm[target] = ''
  }
}
      ...
      <n-form label-placement="left" label-width="140">
        ...
        <n-form-item label="对话模型">
          <n-input v-model:value="config.llm.models.chat" placeholder="普通聊天使用的模型" readonly @click="openModelSelector('models.chat')">
             <template #suffix>
                <n-space :size="4">
                  <n-button size="small" type="error" text @click.stop="clearModel('models.chat')" v-if="config.llm.models.chat">清空</n-button>
        ...
        <n-form-item label="伪人模型">
          <n-input v-model:value="config.llm.models.roleplay" placeholder="模拟真人回复的模型" readonly @click="openModelSelector('models.roleplay')">
             <template #suffix>
                <n-space :size="4">
                  <n-button size="small" type="error" text @click.stop="clearModel('models.roleplay')" v-if="config.llm.models.roleplay">清空</n-button>
        ...
        <n-form-item label="工具调用模型">
          <n-input v-model:value="config.llm.models.toolCall" placeholder="Function Calling 模型" readonly @click="openModelSelector('models.toolCall')">
             <template #suffix>
                <n-space :size="4">
                  <n-button size="small" type="error" text @click.stop="clearModel('models.toolCall')" v-if="config.llm.models.toolCall">清空</n-button>
        ...
        <n-form-item label="搜索模型">
          <n-input v-model:value="config.llm.models.search" placeholder="联网搜索使用的模型" readonly @click="openModelSelector('models.search')">
             <template #suffix>
                <n-space :size="4">
                  <n-button size="small" type="error" text @click.stop="clearModel('models.search')" v-if="config.llm.models.search">清空</n-button>
        ...
        <n-form-item label="思考模型">
          <n-input v-model:value="config.llm.models.reasoning" placeholder="深度推理使用的模型" readonly @click="openModelSelector('models.reasoning')">
             <template #suffix>
                <n-space :size="4">
                  <n-button size="small" type="error" text @click.stop="clearModel('models.reasoning')" v-if="config.llm.models.reasoning">清空</n-button>
